...
parameters="/usr3/graduate/seamorez/GitHub/MSLSP/MSLSP_Parameters_v2.json"
tileList="/usr3/graduate/seamorez/GitHub/MSLSP/tileLists/AK_Toolik.txt"
...
then
    ...
    do
        ...
        qsub $nameArg $logArg_download $downloadArg runDownloadHLS_v2.sh $tile $baseDir $imgStartYr $imgEndYr
    done < $tileList
    ...
    do
        ...
        qsub $nameArg $logArg $nodeArgs $holdArg MSLSP_runTile_SCC_v2.sh $tile $paramName $jobTime
    done < $tileList
else
    while read -r tile
    do
        ...
        nameArg="-N R_${tile}"
        logArg="-o ${logDir}Run_${tile}_${jobTime}.txt"
        qsub $nameArg $logArg $nodeArgs MSLSP_runTile_SCC_v2.sh $tile $paramName $jobTime
    done < $tileList
fi
